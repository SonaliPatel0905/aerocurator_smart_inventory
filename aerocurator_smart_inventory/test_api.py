...
# ── Inventory ────────────────────────────────────────────────
print(f"\n{BAR}\nINVENTORY")
s, d = req("GET", "/inventory", token=admin_token)
inv = d.get("data", [])
test("GET /inventory", s == 200, f"{len(inv)} items")

s, d = req("POST", "/inventory/add", {
    "name": "Test ESC 60A", "category": "Electronics",
    "sku": "TST-ESC-99", "quantity": 30, "price": 44.99,
    "low_stock_threshold": 5
}, token=admin_token)
test("POST /inventory/add (admin)", s == 201, f"id={d.get('data',{}).get('id','?')}")
new_inv_id = d.get("data", {}).get("id")

s, _ = req("POST", "/inventory/add",
           {"name": "Fail", "category": "F", "quantity": 1, "price": 1},
           token=user_token)
test("POST /inventory/add (user -> 403)", s == 403)

if new_inv_id:
    s, d = req("PUT", f"/inventory/update/{new_inv_id}",
               {"quantity": 50, "price": 49.99}, token=admin_token)
    test("PUT /inventory/update", s == 200,
         f"qty={d.get('data',{}).get('quantity','?')}")

    s, d = req("DELETE", f"/inventory/delete/{new_inv_id}", token=admin_token)
    test("DELETE /inventory/delete", s == 200, d.get("message", ""))

s, d = req("GET", "/inventory/categories", token=admin_token)
test("GET /inventory/categories", s == 200, str(d.get("data", [])))

s, d = req("GET", "/inventory?search=Battery", token=admin_token)
test("GET /inventory?search=Battery", s == 200,
     f"{len(d.get('data',[]))} matches")

# ── Purchases ────────────────────────────────────────────────
print(f"\n{BAR}\nPURCHASES")
s, d = req("GET", "/purchases", token=admin_token)
test("GET /purchases", s == 200, f"{len(d.get('data',[]))} records")

s, d = req("POST", "/purchase", {
    "supplier": "TestSupply Co", "component_id": 1, "quantity": 5,
    "cost_per_unit": 85.0, "date": "2026-04-16", "notes": "Test run purchase"
}, token=admin_token)
test("POST /purchase", s == 201,
     f"total={d.get('data',{}).get('total_cost','?')}")

s, d = req("POST", "/purchase", {
    "supplier": "X", "component_id": 1, "quantity": -1, "cost_per_unit": 10
}, token=admin_token)
test("POST /purchase (negative qty -> 400)", s == 400, d.get("message", ""))

# ── Sales ────────────────────────────────────────────────────
print(f"\n{BAR}\nSALES")
s, d = req("GET", "/sales", token=admin_token)
test("GET /sales", s == 200, f"{len(d.get('data',[]))} records")

s, d = req("POST", "/sales", {
    "component_id": 2, "quantity": 3, "price_per_unit": 39.99,
    "date": "2026-04-16", "customer": "Test Aviators Ltd"
}, token=admin_token)
test("POST /sales", s == 201,
     f"total={d.get('data',{}).get('total_price','?')}")

s, d = req("POST", "/sales", {
    "component_id": 2, "quantity": 999999, "price_per_unit": 1
}, token=admin_token)
test("POST /sales (oversell -> 409)", s == 409, d.get("message", "")[:60])

# ── Alerts ───────────────────────────────────────────────────
print(f"\n{BAR}\nALERTS")
s, d = req("GET", "/alerts", token=admin_token)
alerts = d.get("data", [])
test("GET /alerts", s == 200, f"{len(alerts)} low-stock items")
if alerts:
    a = alerts[0]
...
# ── Dashboard & Reports ──────────────────────────────────────
print(f"\n{BAR}\nDASHBOARD & REPORTS")
s, d = req("GET", "/dashboard/stats", token=admin_token)
stats = d.get("data", {})
test("GET /dashboard/stats", s == 200,
...
s, d = req("GET", "/reports/data", token=admin_token)
rdata = d.get("data", {})
test("GET /reports/data", s == 200,
...
s, d = req("GET", "/users", token=admin_token)
users = d.get("data", [])
test("GET /users (admin)", s == 200, f"{len(users)} users")

s, d = req("GET", "/users", token=user_token)
test("GET /users (user -> 403)", s == 403, d.get("message", ""))

runner = next((u for u in users if u.get("email") == "runner@test.io"), None)
if runner:
    rid = runner["id"]
    s, d = req("PUT", f"/users/{rid}/role", {"role": "admin"}, token=admin_token)
    test(f"PUT /users/{rid}/role -> admin", s == 200,
         d.get("data", {}).get("role", "?"))
    s, d = req("PUT", f"/users/{rid}/role", {"role": "user"}, token=admin_token)
    test(f"PUT /users/{rid}/role -> user", s == 200,
         d.get("data", {}).get("role", "?"))
    s, d = req("DELETE", f"/users/{rid}", token=admin_token)
    test(f"DELETE /users/{rid}", s == 200, d.get("message", ""))
else:
...
if me_id:
    s, d = req("DELETE", f"/users/{me_id}", token=admin_token)
...
test("GET /me after logout -> 401", s == 401)
